...
/**
 * SymbolsTreeControl: Generic, keyboard-friendly tree view used on both sides.
 * Responsibilities:
 * - Render symbol nodes with expand/collapse behavior
 * - Handle selection (single/multi/range) and simple filtering
 * - Expose callbacks: nodeSelected, nodeDefaultAction, showIdsChanged
 */
export class SymbolsTreeControl {
    ...
    /** Enable simple text filter wired to input/button elements */
    ...
    /** Toggle showing IDs in labels and rerender */
    ...
    /** Provide new data model; prepares state and refreshes filtering */
    setData(data) {
        this._alNodes = true;
        this._selNode = undefined;
        this._selSymbol = undefined;
    ...
    appendDataHtml(parent, data) {
        if (data) {
            if (data.visible) {            
                ...
                cap.className = "cap";
                if ((this._showIds) && ((data.id) || (data.kind == 315)))
                    cap.innerText  = data.id + ' ' + data.fullName;
                else
                    cap.innerText = data.fullName;
                shead.appendChild(cap);

                if (typeof data.subCount === 'number') {
                    let cnt = document.createElement('div');
                    cnt.className = 'countbadge';
                    cnt.innerText = String(data.subCount);
                    shead.appendChild(cnt);
                }
    ...
    /**
     * Advanced filter: by type list, id, name or fullName (wildcards)
     */
    ...
    /** Select a node with support for Ctrl/Shift semantics */
    ...
    /** Restore selection by visual index path */
